...
type Action = 'status' | 'comment' | 'time' | 'delegate' | 'move' | 'comments' | 'back';
...
export function TaskDetail({ taskId, onAction, onBack }: TaskDetailProps) {
  ...
  return (
    <Box flexDirection="column">
      <Header title={`${t('task.detail_title')} #${task.id}`} subtitle={t('app.press_esc')} />
      ...
      <Box flexDirection="column" marginBottom={1} paddingX={1}>
        ...
        <Box marginTop={1} flexDirection="column">
          ...
          <Box>
            ...
            <Text color="yellow">{task.responsible?.name || `ID: ${task.responsibleId}`}</Text>
          </Box>
          <Box>
          ...
          {task.deadline && (
            <Box>
              ...
              <Text color={new Date(task.deadline) < new Date() ? 'red' : 'green'}>
          ...
          {task.timeSpentInLogs && task.timeSpentInLogs !== '0' && (
            <Box>
              ...
              <Text>{Math.round(Number(task.timeSpentInLogs) / 3600)} {t('task.hours')} {Math.round((Number(task.timeSpentInLogs) % 3600) / 60)} {t('task.minutes')}</Text>
            </Box>
          )}
        </Box>
      </Box>
      ...
      <Box flexDirection="column">
        <Text bold color="cyan">{t('task.actions')}</Text>
        <SelectInput
          items={actions}
