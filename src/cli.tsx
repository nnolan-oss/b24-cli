...
import { setWebhookUrl, setUserId, isAuthenticated, getWebhookUrl, clearConfig } from './utils/config.js';
import { resetClient } from './api/client.js';
import { getCurrentUser } from './api/users.js';
import { t, setLocale, getLocale, getAvailableLocales, LANGUAGE_NAMES } from './i18n/index.js';
...
program
  ...
  .action(() => {
    if (isAuthenticated()) {
      ...
      console.log(`${t('menu.language')}: ${LANGUAGE_NAMES[getLocale()] || getLocale()}`);
    } else {
  ...
  .action((code?: string) => {
    if (code) {
      ...
      if (!available.includes(code)) {
        ...
        console.log(`Available: ${available.map(c => `${c} (${LANGUAGE_NAMES[c] || c})`).join(', ')}`);
        process.exit(1);
      }
      setLocale(code);
