...
export function AddTime({ task, onDone, onBack }: AddTimeProps) {
  ...
  if (success) {
    return (
      <Box flexDirection="column">
        <Text color="green" bold>{t('time.added')} ({hours}{t('task.hours')} {minutes}{t('task.minutes')})</Text>
  ...
  return (
    <Box flexDirection="column">
      <Header title={`${t('time.add_title')} - #${task.id}`} subtitle={t('app.press_esc')} />
      {error && <ErrorMessage message={error} />}

      {step === 'hours' && (
        <Box>
          <Text color="cyan">{t('time.hours')}: </Text>
          <TextInput value={hours} onChange={setHours} onSubmit={() => setStep('minutes')} placeholder="0" />
        </Box>
      )}

      {step === 'minutes' && (
        <Box flexDirection="column">
          <Text dimColor>{t('time.hours')}: {hours}</Text>
          <Box>
            <Text color="cyan">{t('time.minutes')}: </Text>
            <TextInput value={minutes} onChange={setMinutes} onSubmit={() => setStep('comment')} placeholder="0" />
          </Box>
        </Box>
      )}
      ...
      {step === 'comment' && (
        <Box flexDirection="column">
          <Text dimColor>{t('time.hours')}: {hours} | {t('time.minutes')}: {minutes}</Text>
          <Box>
            <Text color="cyan">{t('time.comment')}: </Text>
            <TextInput value={comment} onChange={setComment} onSubmit={() => handleSubmit()} placeholder={t('time.comment_placeholder')} />
          </Box>
